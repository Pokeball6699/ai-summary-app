...
This project can persist uploaded document metadata and generated AI summaries into your Supabase Postgres database.
...
3. The API route `app/api/generate-summary/route.ts` will attempt to persist summaries to the `documents` and `summaries` tables when the service role key is configured.

Security note: never commit service role keys to source control. Use your hosting platform's Secret/Env settings.
